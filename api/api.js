...
app.use(express.static(__dirname + '/static-data/'))
...
app.use((req, res) => {
  let matcher
  if (req.url.match(/\/[\?&#]*$/)) {
    matcher = req.url.split('?')[0].split('/').slice(1, -1)
  } else {
    matcher = req.url.split('?')[0].split('/').slice(1)
  }

  matcher = matcher.map(m => require('querystring').unescape(m))
